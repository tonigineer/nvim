...
local function read_json_file(filepath)
    ...
    if vim.fn.filereadable(expanded) == 0 then
        vim.notify("m3scheme: JSON file not found at " .. expanded, vim.log.levels.ERROR)
        return nil
    end
    ...
    if not ok then
        vim.notify("m3scheme: Failed to parse JSON: " .. tostring(result), vim.log.levels.ERROR)
        return nil
    end
    return result
end
...
local function normalize_colors(colors)
    ...
    c.primary = c.primary or c.primary -- M3 primary
...
local function apply_colors()
    ...
    if not scheme_data or not scheme_data.colours then
        vim.notify("m3scheme: Invalid data from scheme.json.", vim.log.levels.ERROR)
        return
    end
